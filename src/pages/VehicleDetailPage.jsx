...
import { 
  ArrowLeft, MapPin, Share2, Phone, Mail, MessageCircle, 
  Calendar, Gauge, Fuel, Settings, CheckCircle2, ChevronLeft, ChevronRight, Copy, X 
} from 'lucide-react';
...
export default function VehicleDetailPage({ vehicle, onBack }) {
  ...
  const shareText = `Dê uma olhada neste veículo em Ouro Branco: ${vehicle.brand} ${vehicle.model} por ${priceFormatted}.`;
  ...
  const copyToClipboard = () => {
    ...
    alert("Link do veículo copiado com sucesso! Cole nas suas redes sociais.");
  ...
  return (
    <div className="animate-in slide-in-from-bottom-4 fade-in duration-300 pb-12 px-4 md:px-0 max-w-5xl mx-auto relative">
        ...
        >
          ...
          >
            <div className="flex justify-between items-center mb-6">
              <h3 className="font-black text-xl text-slate-800">Compartilhar Veículo</h3>
              <button 
                onClick={() => setIsShareModalOpen(false)} 
      ...
      <div className="flex items-center justify-between mb-6 pt-4">
        <button 
          onClick={onBack} 
          className="flex items-center gap-2 text-slate-500 hover:text-slate-800 transition bg-white px-4 py-2 rounded-xl shadow-sm border border-slate-100"
        >
      ...
      <div className="grid md:grid-cols-3 gap-8">
        ...
        <div className="md:col-span-2 space-y-6">
            ...
            <div className="relative h-[300px] md:h-[450px] bg-slate-900 rounded-2xl overflow-hidden group">
              ...
              {photos.length > 1 && (
                <>
                  <button 
                    onClick={prevImage} 
                    className="absolute left-4 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white text-slate-800 p-2 rounded-full shadow-lg transition opacity-0 group-hover:opacity-100"
                  >
                    <ChevronLeft size={24}/>
                  </button>
                  <button 
                    onClick={nextImage} 
                    className="absolute right-4 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white text-slate-800 p-2 rounded-full shadow-lg transition opacity-0 group-hover:opacity-100"
                  >
                  ...
                  <div className="absolute bottom-4 left-0 right-0 flex justify-center gap-2">
                    {photos.map((_, idx) => (
                      <button 
                        ...
                        className={`w-2.5 h-2.5 rounded-full shadow-sm transition-all ${currentImage === idx ? 'bg-blue-500 w-6' : 'bg-white/60 hover:bg-white'}`} 
                      />
                    ))}
                  </div>
                </>
              ...
              <div className="flex gap-2 overflow-x-auto p-2 mt-1 scrollbar-hide">
                {photos.map((photo, idx) => (
                  <button 
                    key={idx} 
                    onClick={() => setCurrentImage(idx)} 
                    className={`h-16 w-24 shrink-0 rounded-xl overflow-hidden border-2 transition ${currentImage === idx ? 'border-blue-500 opacity-100' : 'border-transparent opacity-60 hover:opacity-100'}`}
                  >
          ...
          <div className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-slate-100">
            <h3 className="text-xl font-bold text-slate-800 mb-4 flex items-center gap-2">Sobre este veículo</h3>
              ...
              <div className="space-y-3">
                ...
                {vehicle.email && (
                  <a 
                    ...
                    className="w-full bg-white hover:bg-slate-50 text-slate-700 border border-slate-200 font-bold py-3 rounded-xl transition flex items-center justify-center gap-2"
                  >
